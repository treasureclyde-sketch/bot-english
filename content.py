...
Слой 3: каждое напоминание — конкретное задание, прямая ссылка, оценка времени
и (в handlers) три кнопки. Треки идут по программе, а не хаотично.

CSCA — это уроки, собранные из учебника самоподготовки CSCA Math (тот же, что и
интерактивный тренажёр в репозитории). По каждой теме: английская экзаменационная
лексика, подробная русская теория с разбором ловушек, формулы, разбор примера,
вопрос на самопроверку и практика на csca.app. Экзамен идёт на английском —
поэтому термины даём по-английски, а объяснение по-русски.

Индекс задания хранится в базе (tracks.content_index) и двигается вперёд на
каждое отправленное напоминание; списки цикличные (пройдёшь все темы — начнёшь
заново, это осознанное интервальное повторение).
"""
...
# --- 6 Minute English -------------------------------------------------------
# Реальные выпуски BBC Learning English «6 Minute English». Список цикличный.
SIX_MIN_EPISODES = [
...
# Нужен для поступления в вузы Китая. Цель — 110-120 (этого хватает большинству
# программ; уточняй минимум конкретного вуза). Тест адаптивный, ~1 ч, балл 10-160.
...
DET_PROGRAM = [
    ...
    ("Interactive Listening / Summarize",
     ...
     "Tip: note the speaker's goal; your summary should answer 'what did they decide?'.",
     20),
    ...
    ("Full adaptive practice test",
     ...
     "текущий балл и слабые секции. Цель 110-120; потом добивай именно слабое.",
...
def six_min_task(index: int):
    """(номер_выпуска, заголовок, ссылка, минуты) для 6min."""
...
def csca_task(index: int):
    """(номер_темы, заголовок, тело_урока, ссылка_практики, минуты) для CSCA.

    Темы цикличны: пройдёшь все — начнёшь заново (интервальное повторение).
    """
    n = len(CSCA_LESSONS)
    title, body, minutes = CSCA_LESSONS[index % n]
    block_no = (index % n) + 1
    return block_no, title, body, CSCA_PRACTICE, minutes


def det_task(index: int):
    """(номер, заголовок, описание, tip, ссылка, минуты) для DET."""
    title, desc, tip, minutes = DET_PROGRAM[index % len(DET_PROGRAM)]
    return index + 1, title, desc, tip, DET_PRACTICE, minutes


def ege_task(index: int):
    """(заголовок, ссылка, минуты) для ЕГЭ."""
