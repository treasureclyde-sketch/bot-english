...
Функционал полностью сохранён: 4 трека (6 Minute English, Duolingo Test,
двуязычные уроки CSCA, Пробник ЕГЭ), кнопки Готово/Позже/Пропустить, перенос
...
- `/duo` — про Duolingo Test и ресурсы для поступления в Китай.

Расписание по умолчанию: 6min раз в 2 дня 08:00, Duolingo раз в 2 дня 17:00,
CSCA раз в 3 дня 19:00, ЕГЭ по субботам 10:00. Воскресенье — выходной.

Существующая база обновляется сама: при первом `/start` после деплоя бот
добавит трек Duolingo и подтянет новые кадентности (`syncTracks`), не трогая
историю и ручные правки времени.
